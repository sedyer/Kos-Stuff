//Program Intial Parameters. Ex: gravity(250,250,0,25). Circular 250x250 orbit with 25 deg inclination.
...
//Note: Setting target periapsis to 0 will burn the final launch stage until its engines flame out.

declare parameter targetedlaunch.
declare parameter inclinedlaunch.	//useless if targetedlaunch is 1.
...
declare function prelaunchsetup{
	...
	//Set up triggers for fairing separation
	...
	if targetedlaunch = 0 {
		if inclinedlaunch < latitude{
			//Kills the program.
			print "ERROR: ORBITAL INCLINATION CANNOT BE LOWER THAN LATITUDE".
			set breaker to 0/0.
		}
...
//Function to handle separation of boosters. May not work on some vehicles. Performs
...
declare function boosterseparation {
	...
	FOR eng IN enginelist {
		if eng:flameout {
			...
			local currentstage to eng:stage.
...
declare function preprogrammedguidance {
	...
	if twr > 1.7
		set pitchstartvelocity to 50.
	else if twr < 1.7 and twr > 1.4
		set pitchstartvelocity to 75.
	else
		set pitchstartvelocity to 100.

	//Set up initial rollover triggers

	lock steering to heading(azimuth, 90).

	when airspeed >= pitchstartvelocity then {
		lock steering to heading(azimuth, 85).
	}.
	when progradepitch <= 85 and airspeed >= pitchstartvelocity then {
...
declare function closedloopguidance{
	...
	set Kp to (.05/twr).	//proportional term
	set Ki to .005.		//integral term
	set Kd to .1.		//derivative term

	set pitchmin to -5.
	lock pchange to Kp*Pz + Ki*I + Kd*D.
	
	if targetedlaunch = 1
		set targetnormalvector to normalvector(target).
	...
	until orbitdone() {
		...
		if apoapsis < tgtapo + 200
			...
			set pitchmin to -5.
		if targetedlaunch = 1 and mod(floor(time:seconds),5) = 0 and floor(time:seconds) <> oldtime{
			set angle to vang(normalvector(ship),targetnormalvector).
			if not (angle > 1){
				set oldtime to floor(time:seconds).
				if vang(targetnormalvector, ship:velocity:orbit) < 90
